...
public class CostPoliciesFindServiceTest extends AbstractServiceTest {
	...
	public CostPoliciesFindServiceTest(String name, PriceUnit priceUnit, Double value, String currency,
			boolean success) {
		// random name
	...
	public void testFind() {
		try {
			...
			if (success) {
				...
				response = service.findById(this.priceProfileCreated.getId());

				for (Message message : response.getMessages()) {
					logger.warn("Error while find Price profile by Id request  [{}] ", message.getMessageText());
				}

				assertNotNull(response);
				assertEquals(false, response.isErrors());
				assertNotNull(response.getResults().getId());
				assertEquals(this.priceProfileCreated.getName(), response.getResults().getName());
