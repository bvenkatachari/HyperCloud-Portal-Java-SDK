...
public class QuotaUpdateServiceTest extends AbstractServiceTest {
	...
	public QuotaUpdateServiceTest(String name, String rpType, int cpu, int memory, int disk) {
	...
	public void testUpdate() throws Exception {
		...
		String modifiedName = quotaCreated.getName() + "_Update";
		// updating name
		quotaCreated.setName(modifiedName);
		logger.info("Update Quota with Name [{}]", quotaCreated.getName());
		response = resourcePoolService.update(quotaCreated);
		for (Message message : response.getMessages()) {
			logger.warn("Error [{}] ", message.getMessageText());
		}
		if (!response.isErrors() && response.getResults() != null) {
			assertEquals(quotaCreated.getName(), response.getResults().getName());
		}
